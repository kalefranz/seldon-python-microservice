...
def start_microservice(app_location):
    ...
    try:
        ...
        p = Popen(cmd, cwd=app_location, env=env_vars,)  # stdout=PIPE, stderr=PIPE,
...
def test_model_template_app():
    with start_microservice(join(dirname(__file__), "model-template-app")):
        ...
        response = requests.get("http://127.0.0.1:5000/predict", params="json=%s" % data)
        response.raise_for_status()
        assert response.json() == {'data': {'names': ['t:0', 't:1'], 'ndarray': [[1.0, 2.0]]}}

        data = ('{"request":{"data":{"names":["a","b"],"ndarray":[[1.0,2.0]]}},'
                '"response":{"meta":{"routing":{"router":0}},"data":{"names":["a","b"],'
                '"ndarray":[[1.0,2.0]]}},"reward":1}')
        response = requests.get("http://127.0.0.1:5000/send-feedback", params="json=%s" % data)
        response.raise_for_status()
...
def test_tester_model_template_app():
    ...
    """
    starting: seldon-microservice-tester /Users/kfranz/ae/seldon-microservice/tests/model-template-app/contract.json 127.0.0.1 5000 --prnt
    ----------------------------------------
    SENDING NEW REQUEST:
